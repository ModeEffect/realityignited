...
abstract class MpdtBaseApi extends WP_REST_Controller {
  ...
  public function get_items( $request ) {
    ...
    $request = $this->before_get_items($args, $request);
  ...
  public function create_item( $request ) {
    ...
    MeprHooks::do_action('mpdt_api_before_create_item', $args, $request);
    ...
    MeprHooks::do_action('mpdt_api_after_create_item', $response, $args, $request);
  ...
  protected function prepare_item_for_database( $request ) {
    ...
    try {
      ...
      throw new Exception("Error creating object: " . $th->getMessage());
    }
  ...
  public function prepare_item_for_response( $item, $request ) {
    ...
    return MeprHooks::apply_filters( 'mepr_developer_tools_rest_item_response', $item, $request );
  }
  ...
  public function apply_accept_fields(Array $_post) {
    foreach($_post as $k => $field) {
      if(!isset($this->utils->accept_fields[$k])) {
        ...
        }else if('bool' === (string) $this->utils->accept_fields[$k]['type']){
          // Check for explicit true values, otherwise default to false
          $_post[$k] = ($_post[$k] === true || strtolower($_post[$k]) === 'true' || $_post[$k] === 1 || $_post[$k] === '1' || strtolower($_post[$k]) === 'yes' || strtolower($_post[$k]) === 'y') ? true : false;
