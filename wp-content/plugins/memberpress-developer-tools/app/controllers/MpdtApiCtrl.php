...
class MpdtApiCtrl extends MpdtBaseCtrl {
  ...
  public function routes() {
    ...
    // Exclude Utils that are course related
    $exclude = ['MpdtAssignmentUtils', 'MpdtCourseUtils', 'MpdtLessonUtils', 'MpdtQuizUtils'];
    foreach($exclude as $ex) {
      $key = array_search(MPDT_UTILS_PATH . "/{$ex}.php", $utils_files);
      if($key !== false) {
        unset($utils_files[$key]);
      }
    }
